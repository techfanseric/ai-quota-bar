...
test('logout invalidates a delayed overview before it can reveal the previous team',async()=>{
 ...
 const context=vm.createContext({Intl,Map,Headers,document:{getElementById:element,addEventListener(){}},fetch:async path=>path.includes('overview')?pending:{ok:true,json:async()=>({ok:true})},URLSearchParams});
