...
//   （struct CodexUsageResponse 及嵌套类型：损失容忍解码语义逐分支镜像——由
    ...
    bool AdditionalRateLimitsDecodeFailed)
{
    ...
    /// <summary>
    /// rate_limit 对象（Swift: RateLimitDetails）。primary/secondary 任一解码失败时该窗口置 null
    /// 并记录 <see cref="PrimaryWindowDecodeFailed"/> / <see cref="SecondaryWindowDecodeFailed"/>
    /// （供 dataConfidence=unknown 判定），另一个窗口不受影响。
    /// </summary>
    public sealed record RateLimitDetails(
        WindowSnapshot? PrimaryWindow,
        WindowSnapshot? SecondaryWindow,
        SpendControlLimitSnapshot? IndividualLimit,
    ...
    /// <summary>
    /// additional_rate_limits 的单个条目（Swift: AdditionalRateLimit）。rate_limit 解码失败
    /// 置 null 并记 <see cref="RateLimitDecodeFailed"/>。
    /// </summary>
    ...
    /// <summary>
    /// credits 对象（Swift: CreditDetails）。has_credits/unlimited 缺失或类型不符按 false；
    /// balance 为数字，或可解析为 double 的字符串（"0"），其余（[]/对象/bool）为 null。
    /// </summary>
    public sealed record CreditDetails(bool HasCredits, bool Unlimited, double? Balance);

    /// <summary>
    /// 个人月度限额（Swift: SpendControlLimitSnapshot）。数值字段走灵活解码（数字/字符串双形态），
    /// resets_at 兼任 wham/usage 的 reset_at 拼写。
    /// </summary>
