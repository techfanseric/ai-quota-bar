...
这一版让菜单栏左右两侧都更容易看懂，也让配额圆环能直接回答：“Codex 还在工作吗？”

- **不用打开菜单，也知道任务有没有在跑。** Codex 工作时，剩余配额环会适当变淡，并按活跃任务数显示 1 至 5 道有方向的能量流；任务结束后恢复为纯白实线。
- **更少扫视，就能找到合适线路。** 单行线路同时展示协议、名称和延迟；最近换线记录左侧说明从哪里切到哪里，右侧显示时间。
- **更快判断 OpenAI 活跃情况。** 线路操作、总速度、最近 60 分钟连接年龄和活跃连接保持纵向阅读，减少无用的次要字段。
- **在不同显示器上也保持原生体验。** 左键配额面板使用真正的 macOS 原生菜单材质，并按打开时所在屏幕调整高度；右键控制中心保持统一、无三角箭头的弹窗边界。

开启 macOS“减少动态效果”后，应用会改用静态任务高亮，不显示游走动画。
