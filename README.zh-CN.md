...
这一版新增了安全、可安装的 Mobile Dashboard，让手机或同一局域网内的另一块屏幕可以持续观察 AI 工作状态。

- **在手机上一眼看完整体状态。** 配额、Codex Activity、线路健康、OpenAI 连接和睡眠保护会在同一页面实时更新。横屏与竖屏下，三个状态板块都会接近三等分，不会因为分辨率变大而缩成小字和大片空白。
- **知道每一个 Codex Task 正在做什么。** 1 至 5 个活跃任务会对应 1 至 5 行独立弹幕；每一行只属于一个真实任务，可读取标题、阶段、项目、分支、模型、推理强度、工具、Token、子任务、时间、进展等生命周期字段。
- **用运动速度表达任务年龄。** 每个任务的字段顺序独立随机，但实时更新时保持稳定；五档明显不同的速度让新任务更快、运行越久的任务越慢。新文字会接到当前字幕后方，不再重置滚动位置。
- **把确认后的 IDLE 变成屏保。** 可选的纯黑白 IDLE 模式会用大小和速度各异的多行字幕铺满屏幕；OLED 防烧屏会轻微移动静态内容，外观可独立选择自动、浅色或深色。
- **精确决定哪些信息离开 Mac。** 最多选择两个配额模型、隐藏账户名称、选择 Codex Activity 背景，并逐项勾选 Task 弹幕字段。详细任务进展只有在要求手动配对且用户明确开启共享后才会提供。
- **安装和配对时不把秘密放进 URL。** 设置页提供二维码、稳定局域网主机名与 IP 备用地址、可选的限时手动配对码、可撤销的安装凭证、严格 Origin 校验和有上限的本地事件流；凭证统一保存在带版本的钥匙串保险库中。
- **长时间无人值守也保持流畅。** 字段更新不会打断弹幕位置，队列数量有上限，任务数变化时会明确释放旧动画和 DOM；IDLE 特效、事件流、定时器与连接轮询都采用有界生命周期，并遵循“减少动态效果”设置。
