...
这一版显著降低紧凑菜单栏的持续开销，同时完善多 Provider 与线路筛选体验。

- **保留 15 FPS 动画，同时摆脱原来的高 CPU。** 紧凑圆环改用预渲染的 `NSStatusBarButton` 帧，不再要求 AppKit 每帧快照整个自定义视图；发布机器实测从约 54–62% 降至多数 8–10%。
- **恢复原来的 Retina 尺寸与悬停细节。** 圆环保持原始点尺寸和间距，悬停显示 Provider 首字母，对角 pace 分隔线也与竖向状态使用相同颜色。
- **同时观察 Codex 与 Kimi 工作状态。** 工作感知选择与任务保护可以在 Provider 活跃时保留各自独立的圆环。
- **更早看懂配额趋势。** 消耗预测与盈余/透支图形帮助判断当前用量能否支撑到重置时间。
- **明确进入 Clash 线路筛选编辑。** 已保存筛选默认保持易读，只有主动点击才进入编辑，并保留正则控制及中英文操作文案。
