...
[v1.23.1 完整发布说明](./docs/releases/v1.23.1.zh-CN.md)

## v1.23.0 · 更低常驻负载，存储与上传可核查

修复菜单栏动画重复生成与历史用量全量处理，保留监测频率和 30 fps 动画。单机十分钟实测平均进程 CPU 约 9.07%，旧版约 95.39%；结果随机器和任务变化。设置新增存储占用、上传分类和云端核验，修复额度上传 HTTP 413。界面只加载最近 35 天，磁盘历史不删除。

[v1.23.0 完整发布说明](./docs/releases/v1.23.0.zh-CN.md)

## v1.22.0 · 更清楚的图标与准确的 GLM 状态

更大的**双向扇形**让菜单栏节奏更容易辨认：上方表示余量，下方表示透支，中点表示正常。顶部开口加宽，让常驻的服务商字母与环端头分开；黄色提示低额度，红色横杠表示不可用。启动、菜单栏、警告弹窗、移动端及官网采用同一套图形。

![浅色和深色下的菜单栏状态](./docs/design/quota-symbol-states.png)

GLM 外环与中心节奏正确跟随所选周周期；5h 即使缺少重置时间，也会按实际采样绘制曲线。支持的个人网页凭证还可只读显示可用重置次数与最近到期时间，不执行兑换。

[自 v1.21.0 以来的完整更新](./docs/releases/v1.22.0.zh-CN.md)，包含团队网页细节改进。

## v1.21.0 有什么变化

用量月历标出额度重置日期，悬停显示具体时间；矩阵气泡跟随所在格子；官网同步复现团队设置与用量图表。[已发布更新日志](https://ai-quota-bar.pages.dev/changelog)。

## v1.20.0 有什么变化

**设置内直接建团，查看和管理团队无需反复登录。**「用量与团队」默认展示创建入口，已加入后也可创建或加入其他团队；团队用量自动加载。所有成员可一键打开只读团队面板，管理者通过本机钥匙串保存的凭据直达管理页；旧团队管理者首次补录一次管理密码即可。

**直观看到谁用了多少：**成员用量条形图、本月日历和最近 24 小时的 288 格五分钟矩阵；点日期查看当天明细，按成员／设备／账号筛选，切换 Tokens、记录数、缓存命中及成本。App 设置与团队网页均可用。

浏览器直达链接有效期两分钟、只能使用一次。成员能查看本团队成员、设备、用量与额度账号，不能删除数据、撤销设备或更新邀请码；跨团队操作仍集中在 `/admin`。本次还修复官网与原生 App 的 UI 还原，包括 hero 热力图格子交叉问题。

[官网](https://ai-quota-bar.pages.dev) · [更新日志](https://ai-quota-bar.pages.dev/changelog) · [完整 v1.20.0 发布说明](./docs/releases/v1.20.0.md)
...
打开「设置 → 用量与团队」创建团队，管理密码自动保存到本机钥匙串，请另存备份，并只把邀请码分享给成员；成员通过邀请码、成员名和成员口令加入。创建或加入后启用成员用量上报和账号配额共享，两项可分别暂停。换台 Mac 使用相同成员口令加入；退出团队会撤销当前设备凭证。

- 无需加入团队也能使用本机统计。
- 「查看团队」直接打开只读团队面板，无需输入密码；成员能互相查看本团队用量、设备和账号额度。
- 「管理团队」使用已保存凭据直达；旧团队管理者首次补录一次管理密码。
- 「创建或加入其他团队」成功后切换本机，已归属旧团队的用量不会转移。
- 其他团队不可读写或删除这些数据。
